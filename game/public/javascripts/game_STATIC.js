...
function preload() {
  ...
  this.load.image('ground', 'images/platformV2.png');
...
function create() {
  ...
  function makeBlock(){
    var wallHeight = Math.floor(Math.random() * (8 - 2) + 2);
    for (var i = 0; i < wallHeight; i++){
      gameState.platforms.create(600, i * 25, 'platform');
    }
  }
  makeBlock();*/
  ...
  // ground
  gameState.ground = this.physics.add.sprite(0, 500, 'ground');
  gameState.ground.setTint(0xff000);
...
function update() {
  ...
  // jump
  if ((gameState.cursors.space.isDown || gameState.cursors.up.isDown) && gameState.player.body.onFloor()) {
